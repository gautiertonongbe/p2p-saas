        ...
        {request.status === "submitted" && isAdmin && (
          <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-amber-800">Aucun workflow configuré au moment de la soumission</p>
                <p className="text-sm text-amber-600 mt-0.5">Configurez un workflow puis relancez l'approbation, ou approuvez directement.</p>
              </div>
            </div>
            <div className="flex items-center gap-2 shrink-0">
              <button onClick={() => setLocation("/workflow-builder")}
                className="px-3 py-2 rounded-xl border border-amber-300 text-amber-700 text-sm font-medium hover:bg-amber-100">
                Configurer →
              </button>
              <button onClick={() => retriggerMut.mutate({ id: request.id })}
                disabled={retriggerMut.isPending}
                className="px-4 py-2 rounded-xl bg-amber-600 text-white text-sm font-semibold hover:bg-amber-700 disabled:opacity-50">
                {retriggerMut.isPending ? "..." : "🔄 Relancer l'approbation"}
              </button>
            </div>
          </div>
        )}
